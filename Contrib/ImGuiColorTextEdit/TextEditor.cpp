...
#ifdef JEVOIS_PRO
            for (int ii = 0; ii < mTabSize; ++ii) EnterCharacter(' ', shift);
#else
			...
			if (!line.empty())
			{
				...
				else
				{
					...
					else
					{
						...
						inComment = (int(commentStartLine) < currentLine || (int(commentStartLine) == currentLine && int(commentStartIndex) <= currentIndex));
  ...
  if (!inited)
  {
    static const char* const keywords[] =
      { "and", "as", "assert", "break", "class", "continue", "def", "del", "elif", "else", "except", "exec",
        ...
        "raise", "return", "try", "while", "with", "yield", "False", "None", "True", "self", "__init__"
      };
    for (auto& k : keywords)
      langDef.mKeywords.insert(k);
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::JeVoisCfg()
{
  static bool inited = false;
  static LanguageDefinition langDef;
  if (!inited)
  {
    static const char* const keywords[] =
      { "help", "help2", "info", "setpar", "getpar", "runscript", "setcam", "getcam",
        "listmappings", "setmapping", "setmapping2", "reload", "streamon", "streamoff",
        "ping", "serlog", "serout", "sync", "date", "shell", "shellstart", "shellstop", "dnnget", "quit",
        "setcamreg", "getcamreg", "setimureg", "getimureg", "setimuregs", "getimuregs",
        "setdmpreg", "getdmpreg", "setdmpregs", "getdmpregs", "restart"
      };
    for (auto& k : keywords)
      langDef.mKeywords.insert(k);
    ...
    static const char* const identifiers[] =
      {
       "RGB3", "RGB24", "YUYV", "GRAY", "RGB32", "BGR24", "BGR32", "MJPG", "BAYER", "BAYER16",
       "BGR3", "BGR24", "JVUI", "NONE", "RGB565", "Crop", "Scale", "CropScale", "Linear", "DOL"
      };
    for (auto& k : identifiers)
    {
      Identifier id;
      id.mDeclaration = "Video format";
      langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
    }
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::CMake()
{
  ...
  if (!inited)
  {
    // from: cmake --help-command-list | awk '{print "\""$1"\","}' | xargs -d'\n'
    static const char* const keywords[] =
      {
