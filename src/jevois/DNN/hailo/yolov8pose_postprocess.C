...
// JEVOIS EDITS: make SCORE_THRESHOLD, IOU_THRESHOLD parameters
...
std::pair<std::vector<KeyPt>, std::vector<PairPairs>> filter_keypoints(std::vector<Decodings> filtered_decodings,
                                                            std::vector<int> network_dims, float joint_threshold) {
    ...
    for (auto& dec : filtered_decodings){
        ...
        // Filter keypoints
        for (int i = 0; i < int(score.shape(0)); i++){
...
void dequantize_box_values16(xt::xarray<float>& dequantized_outputs, int index, 
                        xt::xarray<uint16_t>& quantized_outputs,
                        size_t dim1, size_t dim2, float32_t qp_scale, float32_t qp_zp){
    for (size_t i = 0; i < dim1; i++){
        for (size_t j = 0; j < dim2; j++){
            dequantized_outputs(i, j) = dequantize_value16(quantized_outputs(index, i, j), qp_scale, qp_zp);
        }
    }
}
...
std::vector<Decodings> decode_boxes_and_keypoints(std::vector<HailoTensorPtr> raw_boxes_outputs,
                                                  ...
                                                  int regression_length, float score_threshold) {
    int strided_width = 0, strided_height = 0, class_index = 0;
    ...
    for (uint i = 0; i < raw_boxes_outputs.size(); i++)
    {
        ...
        xt::xarray<uint16_t> output_keypoints = common::get_xtensor_uint16(raw_keypoints[i]);        
        int num_proposals_keypoints = output_keypoints.shape(0) * output_keypoints.shape(1);
        xt::xarray<uint16_t> output_keypoints_quantized = xt::view(output_keypoints, xt::all(), xt::all(), xt::all());
        ...
        for (uint j = 0; j < uint(num_proposals); j++) {
            ...
            // Decode keypoints
            dequantize_box_values16(kpts_corrdinates_and_scores, j, quantized_keypoints, 
