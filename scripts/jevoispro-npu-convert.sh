...
if [[ $task != det && $task != cls && $task != seg && $task != pose && $task != obb ]]; then task="det"; fi
...
case $model in
    yolo*) # ----------------------------------------------------------------------------------------------------
        ...
        case $task in
            ...
            # The generated code for 'einsum' operation is buggy.... need an updated NPU SDK                    
#            worldv2)
#                # We need to patch up the Acuity toolkit for the library compilation to succeed:
#                vxker="../bin/vcmdtools/vsimulator/include/client/vsi_nn_vxkernel.h"
#                if [[ ! -f $vxker ]]; then
#                    echo "JEVOIS: Patching up acuity toolkit for yoyo-world..."
#                    mkdir ../bin/vcmdtools/vsimulator/include/client
#                    ln -s ../ovxlib/libnnext/vsi_nn_vxkernel.h $vxker
#                fi
#                
